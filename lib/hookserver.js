...
function fork() {
    var argv   = process.argv.remove([ '-d', '--daemonize', '--daemonized' ]),
        stdout = fs.openSync(utils.LOG_PATH, 'a')
...
function requestListener(req, res) {
    ...
    if(!pathname || pathname === '/')
        ...
        if(typeof query === 'string' && query.length && (~keys.indexOf(query) || ~keys.indexOf(utils.md5(query))))
                ...
                if(!daemonize) {
                    ...
                    writePid(child.pid, function (err) {
                        if(err) {
                            ...
                            console.log('stdout and stderr goes to "' + utils.LOG_PATH + '"')
