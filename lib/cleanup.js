...
utils.deleteConfigDirectory(function (err) {
    if(err)
        print('Cannot delete "' + utils.APP_HOME + '". Please try running this command again as root/Administrator with the --unsafe-perm flag.')

    try {
        utils.deleteLogFileSync()
    }
    catch(ex) {
        print('Cannot delete "' + utils.LOG_PATH + '". Please try running this command again as root/Administrator with the --unsafe-perm flag.')
    }
